...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="buildCitation">
    ...
    <xsl:variable name="mainTitle">
      ...
      <xsl:choose>
        <xsl:when test="(@type='article' or @type='review') and $mainWork//*"><xsl:sequence select="$title"/></xsl:when>
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test="descendant::t:idno[@type='pi']">
              <a href="/biblio/{t:idno[@type='pi']}/"><xsl:sequence select="$title"/></a>
            </xsl:when>
            <xsl:otherwise><xsl:sequence select="$title"/></xsl:otherwise>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
    <xsl:if test="string-length($author) > 0"><xsl:value-of select="$author"/>, </xsl:if>
  ...
  <xsl:template name="articleTitle">
    <xsl:choose>
      <xsl:when test="t:title[@level='a']">
        <xsl:choose>
          <xsl:when test="descendant::t:idno[@type='pi']">
            <a href="/biblio/{t:idno[@type='pi']}/"><xsl:value-of select="t:title[@level='a']"/></a>
          </xsl:when>
          <xsl:otherwise><xsl:value-of select="t:title[@level='a']"/></xsl:otherwise>
        </xsl:choose>
      </xsl:when>
